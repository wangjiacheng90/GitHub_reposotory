...
(function ($) {
    ...
    BootstrapTable.prototype.initFixedColumns = function () {
    	...
    	if(this.options.rightFixedColumns){
    		if(typeof (this.options.height)!=='undefined'){
	            ...
	            '<div class="fixed-table-header-columns" style="position: absolute;right:17px;">',
	            '<table>',
	            '<thead></thead>',
	            '</table>',
	            '</div>'].join(''));
    			...
    			this.$rightFixedHeader = $([
	            '<div class="fixed-table-header-columns" style="position: absolute;right:0px;">',
	            '<table>',
	            '<thead></thead>',
	            '</table>',
	            '</div>'].join(''));
			...
			if(typeof (this.options.height)!=='undefined'){
    			this.$rightFixedBody = $([
	            '<div class="fixed-table-body-columns" style="position: absolute;right:17px;">',
	            '<table>',
	            '<tbody></tbody>',
	            '</table>',
	            '</div>'].join(''));
    			...
    			this.$rightFixedBody = $([
	            '<div class="fixed-table-body-columns" style="position: absolute;right:0px;">',
	            '<table>',
	            '<tbody></tbody>',
	            '</table>',
	            '</div>'].join(''));
    ...
    BootstrapTable.prototype.initHeader = function () {
		...
		if(this.options.rightFixedColumns){
		    var $trTemp = this.$header.find('tr:eq(0)').clone(),
            $thsTemp = $trTemp.clone().find('th');

			var that = this, $trs = this.$header.find('tr').clone();
	        $trs.each(function () {
	            $(this).find('th:lt(' + ($thsTemp.length-that.options.leftFixedNumber) + ')').remove();
	        });
	        this.$rightFixedHeaderColumns.html('').append($trs); 
		}
    };
    ...
    BootstrapTable.prototype.initBody = function () {
		...
		if(this.options.rightFixedColumns){
	        ...
	        this.$body.find('> tr[data-index]').each(function () {
	            ...
	            $tr.html('');
	            var end = that.options.rightFixedNumber;
	            if (rowspan > 0) {
	                --end;
	                --rowspan;
	            }
    ...
    BootstrapTable.prototype.fitHeaderColumns = function () {
		...
		if (that.options.rightFixedColumns) {
            ...
            if (i >= visibleFields.length - that.options.rightFixedNumber) {
	            ...
	            that.$rightFixedHeader.find('th[data-field="' + visibleFields[index] + '"]')
	                .find('.fht-cell').width($this.innerWidth());
	            headerWidth += $this.outerWidth();
            }
    ...
    BootstrapTable.prototype.fitBodyColumns = function () {
		...
		if(that.options.rightFixedColumns){
	        ...
	        this.$body.find('> tr').each(function (i) {
	            that.$rightFixedBody.find('tr:eq(' + i + ')').height($(this).height() - 1);
	        });
