...
laydate({
 ...
 // 定义几个全局变量做为初始化用===用在table初始化表头时定义field用============================================
           ...
           var fy_data = []; //考核费用曲线数据

           var sjz_data = []; //实际值曲线数据
           var mbz_data = []; //上限值曲线数据
           ...
           function getchart_data(tdmc) {
     	      ...
     	      $.ajax({
     	  		...
     	  		success : function(data) {
     	  		 ...
     	  		 fy_data = data[0].data1; //考核费用曲线数据
     	           sjz_data = data[0].data2; //计划值曲线数据
     	           mbz_data =data[0].data3; //实际值曲线数据
...
//         		alert(getTomorrow(startdate));
         		...
         		msg_comput =msg_comput+ comput_AGCkh_data(startdate) +" \r\n  <br>";
         		...
         		for (var i=0;i<days;i++)
         		{
         			...
         			msg_comput =msg_comput+ comput_AGCkh_data(dayinit) +" \r\n  <br>";
         			document.getElementById('computMsg').innerHTML = msg_comput;
         		}
       ...
       //  进度数据功能
        ...
        function comput_AGCkh_data(rq){
        	var result = '';
        	 $.ajax({
     	  		type : "POST",
     	  		url : 'comput_AGCkh_data.action',
     	  		async: false, //如果不设置这里 将取不到值
     	  	    dataType: "json",
     	  		data : {
     	  			"rq" : rq
